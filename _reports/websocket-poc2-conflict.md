...
편집 시작 시점의 버전과 현재 버전이 다르면 모두 충돌로 처리하려 했다. 하지만 서로 다른 필드를 수정한 경우까지 막으면 자동으로 합칠 수 있는 변경도 사용자가 다시 처리해야 했다.

이후 충돌 여부를 버전 차이 자체가 아니라 사용자가 수정한 필드와 서버에서 바뀐 필드의 교집합으로 판단했다. 장애 중 임시 상태를 보존할 때도 버전만 저장하지 않고 변경 필드를 함께 기록해야 복구 후 안전하게 병합할 수 있다는 기준을 얻었다.
