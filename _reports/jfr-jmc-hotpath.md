...
쿼리 병목을 줄인 뒤에도 지연이 남아 있었지만 코드만 읽어서는 원인을 특정하기 어려웠다. JMC Stack Trace를 따라가면서 모든 요청에서 JWT 검증이 중복 실행되고 있다는 공통 비용을 찾을 수 있었다.

이후 성능 개선에서는 눈에 보이는 쿼리만 확인하지 않고 런타임의 allocation과 호출 경로를 함께 보게 됐다. 구조를 바꾼 뒤에는 병목이 객체 그래프 조립으로 이동했는지도 다시 프로파일링해, 한 번의 개선 결과를 최종 상태로 판단하지 않도록 했다.
