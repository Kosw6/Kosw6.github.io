---
title: "TimescaleDB 시계열 조회 p95 SLO 달성"
layout: single
...
| 운영 목표 | p95 < 300ms @ 300 RPS | **235ms, SLO 달성** |

- **문제**: 동일 복합 인덱스 조건에서도 일반 테이블은 300 RPS의 90일 조회에서 p95 7,247ms 기록
- **개선**: 동일 인덱스 조건에서 하이퍼테이블을 적용하고 시간/공간 청크를 조회 패턴에 맞게 튜닝
...
Trader 서비스의 주 기능은 특정 종목(symb)의 90일치 OHLCV 데이터를 차트로 렌더링하는 것이다.
...
→ 하이퍼테이블 튜닝 후 **p95 235ms**, 공간 파티션 4 기준 SLO 충족
...
복합 인덱스만 추가하면 조회 지연을 해결할 수 있다고 생각했다. 하지만 같은 인덱스 조건에서 다시 비교하자 쿼리 경로뿐 아니라 조회 범위와 chunk 구조가 스캔 대상과 planning 비용을 결정한다는 점을 확인했다.

이후 시계열 조회를 개선할 때는 인덱스 하나의 효과로 결론 내리지 않고, 실제 조회 범위에 맞춘 chunk pruning과 웜·콜드 구간의 차이를 함께 확인하는 것을 기준으로 삼았다.
