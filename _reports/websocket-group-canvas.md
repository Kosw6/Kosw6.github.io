...
발생한 메시지를 모두 전달하는 것이 가장 안전하다고 생각했다. 하지만 커서처럼 빠르게 바뀌는 상태는 오래된 값을 모두 보내는 것보다 최신값을 제때 전달하는 편이 사용자에게 더 중요했다.

이후 실시간 이벤트를 휘발성과 비휘발성으로 나누고 서로 다른 전달 기준을 적용했다. 수신 건수만으로는 중복 재전송을 정상 처리량으로 오해할 수 있어, 지연 구간과 실제 상태 변화 건수를 함께 확인하는 것을 검증 기준으로 삼았다.
