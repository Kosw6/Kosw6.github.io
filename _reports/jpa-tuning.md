...
필요한 필드만 조회하는 Projection이 Fetch Join보다 가벼울 것으로 예상했다. 실제 부하에서는 DTO 생성량과 Payload 크기가 GC와 직렬화 비용으로 이어져 반대 결과가 나왔다.

이후 조회 전략을 선택할 때는 쿼리 수만 보지 않고 생성되는 객체 수, 반환 Payload, GC와 tail latency를 함께 확인했다. 한 병목을 줄인 뒤에는 다음 병목이 DB에서 애플리케이션과 네트워크로 이동할 수 있으므로 같은 조건에서 다시 측정하는 것을 기준으로 삼았다.
